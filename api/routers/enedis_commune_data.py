...
from ..models.enedis_commune import CommuneIn, CommuneOut, CommuneUpdate

COLL_COMMUNE = "enedis_commune_data"
...
# ---------- DEBUG ----------
@router.get("/debug/one", response_model=CommuneOut)
async def debug_one():
    doc = await db[COLL_COMMUNE].find_one({})
    if not doc:
...
async def list_communes(
    ...
    if code_commune:
        ands.append({"$or": [{"code_commune": code_commune}, {"code_commune": _num_or_str(code_commune)}]})
    if code_epci:
    ...
    if nom_commune:
        ands.append({"nom_commune": nom_commune})

    q: Dict[str, Any] = {"$and": ands} if ands else {}

    cursor = db[COLL_COMMUNE].find(q)
...
# ---------- SAMPLE ----------
@router.get("/sample", response_model=List[CommuneOut])
async def sample_communes(
    ...
    q: Dict[str, Any] = {"$and": ands} if ands else {}
    docs = await db[COLL_COMMUNE].find(q).limit(limit).to_list(length=limit)
    return [_to_str_id(d) for d in docs]
...
async def count_communes(
    ...
    n = await db[COLL_COMMUNE].count_documents(q)
    return {"count": n, "query": q}

# ---------- BY KEY (annee + code_commune) ----------
@router.get("/by_key/{annee}/{code_commune}", response_model=CommuneOut)
async def get_by_key(annee: int, code_commune: str = Path(..., description="число или строка")):
    doc = await db[COLL_COMMUNE].find_one({
        "annee": annee,
        "$or": [{"code_commune": code_commune}, {"code_commune": _num_or_str(code_commune)}],
    })
    if not doc:
...
# ---------- GET BY _id ----------
@router.get("/{doc_id}", response_model=CommuneOut)
async def get_commune(doc_id: str = Path(..., description="Mongo ObjectId (24 hex)")):
    try:
        _id = ObjectId(doc_id)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid id")
    doc = await db[COLL_COMMUNE].find_one({"_id": _id})
    if not doc:
        raise HTTPException(status_code=404, detail="Not found")
    return _to_str_id(doc)

# ---------- CREATE ----------
@router.post("", response_model=CommuneOut, status_code=201)
async def create_commune(payload: CommuneIn):
    res = await db[COLL_COMMUNE].insert_one(payload.model_dump(exclude_none=True))
    doc = await db[COLL_COMMUNE].find_one({"_id": res.inserted_id})
    return _to_str_id(doc)

# ---------- BULK INSERT ----------
@router.post("/bulk", response_model=List[CommuneOut], status_code=201)
async def bulk_insert(items: List[CommuneIn]):
    if not items:
        return []
    docs = [i.model_dump(exclude_none=True) for i in items]
    res = await db[COLL_COMMUNE].insert_many(docs)
    out = await db[COLL_COMMUNE].find({"_id": {"$in": res.inserted_ids}}).to_list(length=len(res.inserted_ids))
    return [_to_str_id(d) for d in out]

# ---------- PATCH ----------
@router.patch("/{doc_id}", response_model=CommuneOut)
async def update_commune(doc_id: str, payload: CommuneUpdate):
    try:
        _id = ObjectId(doc_id)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid id")
    ...
    if data:
        await db[COLL_COMMUNE].update_one({"_id": _id}, {"$set": data})
    doc = await db[COLL_COMMUNE].find_one({"_id": _id})
    if not doc:
        raise HTTPException(status_code=404, detail="Not found")
    return _to_str_id(doc)
...
async def delete_commune(doc_id: str):
    try:
        _id = ObjectId(doc_id)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid id")
    res = await db[COLL_COMMUNE].delete_one({"_id": _id})
